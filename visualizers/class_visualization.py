...
class ClassVisualization:
    ...
    def create_class_visualization(self, target_y, class_names, model, dtype, **kwargs):
        ...
        for t in range(num_iterations):
            ...
            y_score = scores[:, target_y]
            y_score_w_reg = y_score - l2_reg * torch.sum(img_var ** 2)
            # y_score_w_reg = y_score
            y_score_w_reg.backward()
            img_var.data += learning_rate * img_var.grad / torch.norm(img_var.grad)
            img_var.grad.zero_()
            if t%10 == 0:
                print('y_score_w_reg.grad.sum()', 'y_score_w_reg.sum()','l2_reg * torch.sum(img_var ** 2)',  l2_reg*torch.sum(img_var ** 2))
            ########################################################################
            #                             END OF YOUR CODE                         #
            ########################################################################
