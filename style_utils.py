...
def style_transfer(name, content_image, style_image, image_size, style_size, content_layer, content_weight,
    ...
    for t in range(200):
        ...
        # cl = content_loss.forward(content_weight, img, content_layer)
        cl = content_loss(content_weight,feats[content_layer], content_target)

        # sl = style_loss.forward(feats, style_layers, style_targets, style_weights)
        ...
        # print('t = ',t,' - total_loss ', total_loss)
        # total_loss.backward()
        if t < decay_lr_at:
            lr = initial_lr
        else:
        ...
        for param in optimizer.param_groups:
            param['lr'] = lr
        # img_var.data += lr * img_var.grad / torch.norm(img_var.grad)
        # img.data += lr * img_var.grad / torch.norm(img_var.grad)
        # img_var.grad.zero_()
